...
if( ! empty($row_type) && $row_type == 'parallax'){
	...
	if(search_and_go_elated_options()->getOptionValue('parallax_on_off') == 'off'){
...
if($row_type == 'row' && $css_animation != ''){
	...
	if($transition_delay !== ''){
		...
		$inner_start .= '<div '.search_and_go_elated_get_inline_style($animation_styles).'>';
		$inner_end .= '</div>';
	}else{
		$inner_start .= '<div>';
		$inner_end .= '</div>';
...
if($video == 'show_video'){
	...
	if($video_image) {
		$video_output .= '<div class="eltd-mobile-video-image" ' . search_and_go_elated_get_inline_attr($video_mobile_style, 'style') . ')"></div>';
	}
	$video_output .= '<div ' . search_and_go_elated_get_class_attribute($video_overlay_class) . search_and_go_elated_get_inline_attr($video_overlay_style, 'style') . '></div>';
	$video_output .= '<div class="eltd-video-wrap">';
	$video_output .= '<video class="eltd-video" width="1920" height="800" '. search_and_go_elated_get_inline_attr($video_attrs, 'poster')  .' controls="controls" preload="auto" loop autoplay muted>';
	...
	$video_output .='<object width="320" height="240" type="application/x-shockwave-flash" data="'.ELATED_ASSETS_ROOT.'/js/flashmediaelement.swf">';
		$video_output .='<param name="movie" value="'.ELATED_ASSETS_ROOT.'/js/flashmediaelement.swf" />';
		if(!empty($video_mp4)) {
			$video_output .= '<param name="flashvars" value="controls=true&amp;file=' . $video_mp4 . '" />';
		}
	if($v_image) {
		$video_output .= '<img ' . search_and_go_elated_get_inline_attr($v_image, 'src') . ' width="1920" height="800" title="No video playback capabilities" alt="'.esc_html__('video thumb','search-and-go').'" />';
	}
	$video_output .='</object>';
	$video_output .='</video>';
	$video_output .='</div>';
