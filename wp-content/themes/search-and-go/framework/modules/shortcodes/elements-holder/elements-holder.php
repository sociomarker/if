<?php
namespace SearchAndGoElated\Modules\Shortcodes\ElementsHolder;
...
class ElementsHolder implements ShortcodeInterface{
	...
	public function vcMap() {
		vc_map( array(
			'name' => esc_html__('Elated Elements Holder', 'search-and-go'),
			'base' => $this->base,
	...
	public function render($atts, $content = null) {
		...
		if($show_border === 'no'){
			$elements_holder_style .= 'border-width:0;';
		}
		if($border_color !== ''){
			$elements_holder_style .= 'border-color: '.$border_color.';';
		}

		$elements_holder_class = implode(' ', $elements_holder_classes);

		$html .= '<div ' . search_and_go_elated_get_class_attribute($elements_holder_class) . ' ' . search_and_go_elated_get_inline_attr($elements_holder_style, 'style'). '>';
