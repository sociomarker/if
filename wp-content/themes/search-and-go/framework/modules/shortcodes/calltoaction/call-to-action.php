<?php
namespace SearchAndGoElated\Modules\Shortcodes\CallToAction;
...
class CallToAction implements ShortcodeInterface {
	...
	public function vcMap() {
		...
		$call_to_action_button_IconCollections = search_and_go_elated_icon_collections()->iconCollections;
		...
		vc_map( array(
				'name' => esc_html__('Elated Call To Action', 'search-and-go'),
				'base' => $this->getBase(),
				'category' => 'by ELATED',
				...
				'params' => array_merge(
					...
					search_and_go_elated_icon_collections()->getVCParamsArray(array('element' => 'type', 'value' => array('with-icon'))),
					array(
						array(
							'type' 			=> 'textfield',
						...
						array(
							...
							'value' => array_merge(array('No Icon' => ''),search_and_go_elated_icon_collections()->getIconCollectionsVC()),
							'save_always' => true,
							'dependency' => array('element' => 'show_button', 'value' => array('yes'))
						)
					),
	...
	public function render($atts, $content = null) {
		...
		foreach (search_and_go_elated_icon_collections()->iconCollections as $collection_key => $collection) {

			$call_to_action_icons_form_fields['button_' . $collection->param ] = '';

		}

		$args = array_merge($args, search_and_go_elated_icon_collections()->getShortcodeParams(),$call_to_action_icons_form_fields);
		...
		$html = search_and_go_elated_get_shortcode_module_template_part('templates/call-to-action-template', 'calltoaction', '', $params);
	...
	private function getCallToActionIcon($params) {
		...
		$icon = search_and_go_elated_icon_collections()->getIconCollectionParamNameByKey($params['icon_pack']);
		$iconStyles = array();
		$iconStyles['icon_attributes']['style'] = $this->getIconStyles($params);
		$call_to_action_icon = '';
		if(!empty($params[$icon])){			
			$call_to_action_icon = search_and_go_elated_icon_collections()->renderIcon( $params[$icon], $params['icon_pack'], $iconStyles );
		}
	...
	private function getButtonParameters($params) {
		...
		if(!empty($params['button_icon_pack'])) {
			...
			$iconPackName = search_and_go_elated_icon_collections()->getIconCollectionParamNameByKey($params['button_icon_pack']);
