<?php
namespace SearchAndGoElated\Modules\Shortcodes\Icon;

use SearchAndGoElated\Modules\Shortcodes\Lib\ShortcodeInterface;

/**
 * Class Icon
 * @package SearchAndGoElated\Modules\Shortcodes\Icon
 */
class Icon implements ShortcodeInterface {
    ...
    public function vcMap() {
        vc_map(array(
            'name'                      => esc_html__('Elated Icon', 'search-and-go'),
            'base'                      => $this->base,
            'category'                  => 'by ELATED',
            'icon'                      => 'icon-wpb-icons extended-custom-icon',
            'allowed_container_element' => 'vc_row',
            'params'                    => array_merge(
                \SearchAndGoIconCollections::get_instance()->getVCParamsArray(),
                array(
                    array(
                        'type'        => 'dropdown',
                        'heading'     => 'Size',
                    ...
                    array(
                        ...
                        'description' => esc_html__('Please insert border radius(Rounded corners) in px. For example: 4 ', 'search-and-go'),
    ...
    public function render($atts, $content = null) {
        ...
        $default_atts = array_merge($default_atts, search_and_go_elated_icon_collections()->getShortcodeParams());
        ...
        $html = search_and_go_elated_get_shortcode_module_template_part('templates/icon', 'icon', '', $params);
        ...
        if(($params['type'] !== 'normal' && !empty($params['shape_size'])) ||
           ($params['type'] == 'normal')
            ...
            if(!empty($params['custom_size'])) {
                $iconStyles[] = 'font-size:'.search_and_go_elated_filter_px($params['custom_size']).'px';
            }
        }
    ...
    private function generateIconHolderStyles($params) {
        ...
        if(isset($params['type']) && $params['type'] !== 'normal') {
            ...
            if(!empty($shapeSize)) {
                $iconHolderStyles[] = 'width: '.search_and_go_elated_filter_px($shapeSize).'px';
                $iconHolderStyles[] = 'height: '.search_and_go_elated_filter_px($shapeSize).'px';
                $iconHolderStyles[] = 'line-height: '.search_and_go_elated_filter_px($shapeSize).'px';
            }
            ...
            if(!empty($params['border_color']) && (isset($params['border_width']) && $params['border_width'] !== '')) {
				...
				$iconHolderStyles[] = 'border-width: '.search_and_go_elated_filter_px($params['border_width']).'px';
            ...
            if($params['type'] == 'square') {
                if(isset($params['border_radius']) && $params['border_radius'] !== '') {
                    $iconHolderStyles[] = 'border-radius: '.search_and_go_elated_filter_px($params['border_radius']).'px';
                }
            }
        }
