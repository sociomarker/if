...
if(!function_exists('search_and_go_elated_get_title')) {
    /**
     * Loads title area HTML
     */
    function search_and_go_elated_get_title() {
        $id = search_and_go_elated_get_page_id();

        extract(search_and_go_elated_title_area_height());
        extract(search_and_go_elated_title_area_background());
        ...
        if(($meta_temp = get_post_meta($id, "eltd_show_title_area_meta", true)) !== ""){
            ...
            $show_title_area = search_and_go_elated_options()->getOptionValue('show_title_area') == 'yes' ? true : false;
        }
        ...
        if(($meta_temp = get_post_meta($id, "eltd_title_area_type_meta", true)) !== ""){
            ...
            $type = search_and_go_elated_options()->getOptionValue('title_area_type');
        }
        ...
        if(($meta_temp = get_post_meta($id, "eltd_title_area_enable_breadcrumbs_meta", true)) !== ""){
            ...
            $enable_breadcrumbs = search_and_go_elated_options()->getOptionValue('title_area_enable_breadcrumbs') == 'yes' ? true : false;
        }
		$enable_title_text = search_and_go_elated_get_meta_field_intersect('title_area_show_title_text', $id);
        ...
        $parameters = apply_filters('search_and_go_elated_title_area_height_params', $parameters);

        search_and_go_elated_get_module_template_part('templates/title', 'title', '', $parameters);
    }
}

if(!function_exists('search_and_go_elated_get_title_text')) {
    /**
     * Function that returns current page title text. Defines search_and_go_elated_title_text filter
     ...
     * @see search_and_go_elated_is_woocommerce_installed()
     */
    function search_and_go_elated_get_title_text() {
        ...
        if (is_tag()) {
            ...
            $title = single_term_title("", false).esc_html__(' Tag', 'search-and-go');
        }
        ...
        elseif (is_author()) {
            ...
            $title = esc_html__('Author:', 'search-and-go') . " " . get_the_author();
        }
        ...
        elseif (is_search()) {
            ...
            $title = esc_html__('Search', 'search-and-go');
        }

        //is current page 404?
        elseif (is_404()) {
            //is 404 title text set in theme options?
            if(search_and_go_elated_options()->getOptionValue('404_title') != "") {
                //get it from options
                $title = search_and_go_elated_options()->getOptionValue('404_title');
            } else {
                //get default 404 page title
                $title = esc_html__('404 - Page not found', 'search-and-go');
            }
        }

        //is WooCommerce installed and is shop or single product page?
        elseif(search_and_go_elated_is_woocommerce_installed() && (is_shop() || is_singular('product'))) {
        ...
        elseif(search_and_go_elated_is_woocommerce_installed() && (is_product_category() || is_product_tag())) {
            global $wp_query;
        ...
        elseif (is_archive()) {
            $title = esc_html__('Archive','search-and-go');
        }
        ...
        $title = apply_filters('search_and_go_elated_title_text', $title);

        return $title;
    }
}

if(!function_exists('search_and_go_elated_title_text')) {
    /**
     * Function that echoes title text.
     *
     * @see search_and_go_elated_get_title_text()
     */
    function search_and_go_elated_title_text() {
        echo search_and_go_elated_get_title_text();
    }
}

if(!function_exists('search_and_go_elated_subtitle_text')) {
    /**
     * Function that echoes subtitle text.
     *
     */
    function search_and_go_elated_subtitle_text() {
        $id = search_and_go_elated_get_page_id();
...
if(!function_exists('search_and_go_elated_custom_breadcrumbs')) {
    /**
     ...
     * @see is_front_page()
     * @see is_category()
     * @see search_and_go_elated_is_product_category()
     * @see get_search_query()
     */
    function search_and_go_elated_custom_breadcrumbs() {
        global $post, $wp_query;

        $output = "";
        $homeLink = esc_url(home_url('/'));
        $pageid = search_and_go_elated_get_page_id();
        $bread_style = "";
        ...
        $delimiter = '<span class="eltd-delimiter" '.search_and_go_elated_get_inline_style($bread_style).'>&nbsp;/&nbsp;</span>'; // delimiter between crumbs
        $home = get_bloginfo('name'); // text for the 'Home' link
        $showCurrent = 1; // 1 - show current post/page title in breadcrumbs, 0 - don't show
        $before = '<span class="eltd-current" '.search_and_go_elated_get_inline_style($bread_style).'>'; // tag before the current crumb
        $after = '</span>'; // tag after the current crumb

        if (is_home() && !is_front_page()) {
            $output = '<div class="eltd-breadcrumbs"><div class="eltd-breadcrumbs-inner"><a '.search_and_go_elated_get_inline_style($bread_style).' href="' . $homeLink . '">' . $home . '</a>' . $delimiter . ' <a '.search_and_go_elated_get_inline_style($bread_style).' href="' . $homeLink . '">'. get_the_title($pageid) .'</a></div></div>';
        ...
        elseif(is_front_page()) {
            if ($showOnHome == 1) $output = '<div class="eltd-breadcrumbs"><div class="eltd-breadcrumbs-inner"><a '.search_and_go_elated_get_inline_style($bread_style).' href="' . $homeLink . '">' . $home . '</a></div></div>';
        }

        else {

            $output .= '<div class="eltd-breadcrumbs"><div class="eltd-breadcrumbs-inner"><a '.search_and_go_elated_get_inline_style($bread_style).' href="' . $homeLink . '">' . $home . '</a>' . $delimiter;

            if ( is_category() || search_and_go_elated_is_product_category()) {
                ...
                $output .= $before . esc_html__('Search results for ', 'search-and-go') . '"' . get_search_query() . '"' . $after;

            } elseif ( is_day() ) {
                $output .= '<a '.search_and_go_elated_get_inline_style($bread_style).' href="' . get_year_link(get_the_time('Y')) . '">' . get_the_time('Y') . '</a>' . $delimiter;
                $output .= '<a '.search_and_go_elated_get_inline_style($bread_style).' href="' . get_month_link(get_the_time('Y'),get_the_time('m')) . '">' . get_the_time('F') . '</a>' . $delimiter;
                ...
                $output .= '<a '.search_and_go_elated_get_inline_style($bread_style).' href="' . get_permalink($parent) . '">' . $parent->post_title . '</a>';
                ...
                while ($parent_id) {
                    ...
                    $breadcrumbs[] = '<a '.search_and_go_elated_get_inline_style($bread_style).' href="' . get_permalink($page->ID) . '">' . get_the_title($page->ID) . '</a>';
                ...
                $output .= $before . esc_html__('Posts tagged ','search-and-go') .'"' . single_tag_title('', false) . '"' . $after;

            } elseif ( is_author() ) {
                global $authordata;
                $output .= $before . esc_html__('Articles posted by ','search-and-go') . $authordata->display_name . $after;
            ...
            if ( get_query_var('paged') ) {
                ...
                $output .= $before . " (" . esc_html__('Page', 'search-and-go') . ' ' . get_query_var('paged') . ")" . $after;
...
if(!function_exists('search_and_go_elated_get_title_area_height')) {

    /**
     * Function that returns title height
     **/
    function search_and_go_elated_get_title_area_height() {
        ...
        if(($meta_temp = get_post_meta($id, 'eltd_title_area_type_meta', true)) != '') {
            ...
            $title_type = search_and_go_elated_options()->getOptionValue('title_area_type');
        }

        $title_height = 220; // default title height without header height
        ...
        } elseif(search_and_go_elated_options()->getOptionValue('title_area_height') !== '') {
            $title_height = search_and_go_elated_options()->getOptionValue('title_area_height');
        }

        return apply_filters('search_and_go_elated_title_area_height', $title_height);
    }
}

if(!function_exists('search_and_go_elated_get_title_content_padding')) {
    /**
     * Function that returns title content pading
     **/

    function search_and_go_elated_get_title_content_padding() {
        return apply_filters('search_and_go_elated_title_content_padding', 0);
    }
}


if(!function_exists('search_and_go_elated_title_area_height')) {
    /**
     * Function that returns title height and padding to be applied in template
     **/

    function search_and_go_elated_title_area_height() {
        $id = search_and_go_elated_get_page_id();
        $title_height_and_padding = array();
        $title_height          = search_and_go_elated_get_title_area_height();
        $header_height_padding = search_and_go_elated_get_title_content_padding();
        ...
        if(($meta_temp = get_post_meta($id, "eltd_title_area_background_image_responsive_meta", true)) != "") {
            ...
            $is_img_responsive = search_and_go_elated_options()->getOptionValue('title_area_background_image_responsive');
        }

        if(($meta_temp = get_post_meta($id, "eltd_title_area_vertial_alignment_meta", true)) !== '') {
            $title_vertical_alignment = $meta_temp;
        }else{
            $title_vertical_alignment = search_and_go_elated_options()->getOptionValue('title_area_vertial_alignment');
        }
...
if(!function_exists('search_and_go_elated_title_area_background')) {
    /**
     * Function that returns title background style be applied in template
     **/

    function search_and_go_elated_title_area_background() {
        $id = search_and_go_elated_get_page_id();
        $show_title_img = true;
        ...
        if(($meta_temp = get_post_meta($id, "eltd_title_area_background_color_meta", true)) != ""){
            ...
            $background_color = search_and_go_elated_options()->getOptionValue('title_area_background_color');
        }
        ...
        if(($meta_temp = get_post_meta($id, "eltd_title_area_background_image_meta", true)) != ""){
            ...
            $background_image = search_and_go_elated_options()->getOptionValue('title_area_background_image');
        }

        //check for background image width
        $background_image_width = "";
        if($background_image !== ''){
            $background_image_width_dimensions_array = search_and_go_elated_get_image_dimensions($background_image);
