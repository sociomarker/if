...
if( ! class_exists('SearchAndGoSidebar') ){
	
	class SearchAndGoSidebar{
		...
		function __construct(){
			...
			$this->title = esc_html__('Custom Widget Area','search-and-go');
		...
		function template_add_widget_field(){
			...
			echo "\n    <span class='input_wrap'><input type='text' value='' placeholder = '".esc_html__('Enter Name of the new Widget Area', 'search-and-go')."' name='eltd-add-widget' /></span>";
			echo "\n    <input class='button' type='submit' value='".esc_html__('Add Widget Area', 'search-and-go')."' />";
			echo "\n    ".$nonce;
			echo "\n  </form>";
			echo "\n</script>\n";
		}
		...
		function register_custom_sidebars(){
			...
			$args = apply_filters('search_and_go_elated_custom_widget_args', $args);
