...
class SearchAndGoImport {
    ...
    public function import_content($file){
        if (!class_exists('WP_Importer')) {
            ...
            $this->message = esc_html__("Error loading files", "eltd_core");
        }
    }
    ...
    public function import_options($file){
        $options = $this->file_options($file);
        update_option( 'eltd_options_search_and_go', $options);
    ...
    function eltd_admin_import() {
        if (eltd_core_theme_installed()) {
            ...
            $this->pagehook = add_submenu_page(
                ...
                array($search_and_go_elated_Framework->getSkin(), 'renderImport')
            );

            add_action('admin_print_scripts-'.$this->pagehook, 'search_and_go_elated_enqueue_admin_scripts');
            add_action('admin_print_styles-'.$this->pagehook, 'search_and_go_elated_enqueue_admin_styles');
...
function eltd_init_import_object(){
    global $search_and_go_elated_import_object;
    $search_and_go_elated_import_object = new SearchAndGoImport();
}

add_action('init', 'eltd_init_import_object');

if(!function_exists('search_and_go_elated_dataImport')){
    function search_and_go_elated_dataImport(){
        global $search_and_go_elated_import_object;

        if ($_POST['import_attachments'] == 1)
            $search_and_go_elated_import_object->attachments = true;
        else
            $search_and_go_elated_import_object->attachments = false;

        $folder = "searchandgo/";
        if (!empty($_POST['example']))
            $folder = $_POST['example']."/";

        $search_and_go_elated_import_object->import_content($folder.$_POST['xml']);

        die();
    }

    add_action('wp_ajax_eltd_dataImport', 'search_and_go_elated_dataImport');
}

if(!function_exists('search_and_go_elated_widgetsImport')){
    function search_and_go_elated_widgetsImport(){
        global $search_and_go_elated_import_object;
        ...
        $search_and_go_elated_import_object->import_widgets($folder.'widgets.txt',$folder.'custom_sidebars.txt');

        die();
    }

    add_action('wp_ajax_eltd_widgetsImport', 'search_and_go_elated_widgetsImport');
}

if(!function_exists('search_and_go_elated_optionsImport')){
    function search_and_go_elated_optionsImport(){
        global $search_and_go_elated_import_object;
        ...
        $search_and_go_elated_import_object->import_options($folder.'options.txt');

        die();
    }

    add_action('wp_ajax_eltd_optionsImport', 'search_and_go_elated_optionsImport');
}

if(!function_exists('search_and_go_elated_otherImport')){
    function search_and_go_elated_otherImport(){
        global $search_and_go_elated_import_object;
        ...
        $search_and_go_elated_import_object->import_options($folder.'options.txt');
        $search_and_go_elated_import_object->import_widgets($folder.'widgets.txt',$folder.'custom_sidebars.txt');
        $search_and_go_elated_import_object->import_menus($folder.'menus.txt');
        $search_and_go_elated_import_object->import_settings_pages($folder.'settingpages.txt');

        die();
    }

    add_action('wp_ajax_eltd_otherImport', 'search_and_go_elated_otherImport');
}